...
class QuadraticPdeoptStationaryCoerciveReductor(CoerciveRBReductor):
    ...
    def reduce_to_subbasis(self, dim):
        if dim == len(self.RBPrimal):
            ...
            reductor = QuadraticPdeoptStationaryCoerciveReductor(self.fom, self.RBPrimal[:dim],
                 ...
                 self.prepare_for_gradient_estimate, self.adjoint_estimate)
    ...
    def extend_adaptive_taylor(self, mu, U = None, P = None):
        ...
        an, bn = len(self.RBPrimal), len(self.RBDual)
        if self.unique_basis:
            print('Length of basis is {}'.format(an,bn))
        else:
    ...
    def project_hessian(self):
        ...
        for (key, size) in sorted(self.primal_fom.parameters.items()):
            hessian = np.empty(size, dtype=dict)
            for l in range(size):
                D_rhs = self.projected_output['dual_projected_rhs'].d_mu(key,l)
                D_P_op = self.projected_output['dual_primal_projected_op'].d_mu(key,l)
                proj_hessian = {
                    'D_rhs' : D_rhs,
                    'D_P_op' : D_P_op,
                    }
